...
const formSendEvent = new Event('formSend');
...
function send(object, url, form) {
    ...
    fetch(url, {
        ...
        if (res.match(/11/)) {
            ...
            resetForm(form);
            form.inputsData = object;
            form.dispatchEvent(formSendEvent);
        } else {
            sendMessageStatus(form, 'Помилка відправки');
        }
        setTimeout(() => {
...
document.querySelector('form').addEventListener('formSend', function(evt) {

    // console.log(JSON.stringify(evt.target.inputsData));
    istat(evt.target.inputsData);

    // evt.target.inputsData;
});

function istat(textData) {
    const URL = 'https://istat24.com/event_import/istat',
        sid_id = 1426,
        target_id = 330,
        googleClientID = window.googleClientId;
    let googleClientIdString = '';
    googleClientID !== undefined ?
        googleClientIdString = `&google_client_id=${googleClientID}` :
        null;
    headers = {
        method: 'POST',
    };
    fetch(`${URL}?sid=${sid_id}target_id=${target_id}${googleClientIdString}&text_data=${JSON.stringify(textData)}`, headers)
        .then(res => res.json())
